...
**Hindi-English code-mixed ("Hinglish") text.** Real Indian scam and transaction messages are routinely code-mixed, not clean English — so both classifiers were tested against realistic Hinglish, not just the English holdout/stress sets. Measurement first surfaced a real, non-hypothetical gate problem: `src/pipeline/text_guard.py` ran every message through `langdetect`, restricted to English only, and that gate rejected **46-72% of realistic Hinglish messages** as "not English" — mostly misidentifying romanized Hindi as Indonesian, Estonian, or Turkish (short-text n-gram collisions), and doing so **non-deterministically**, since `langdetect` ships with no fixed random seed. **Fixed**: `text_guard.py` now checks directly for unambiguous Hindi grammatical markers (common postpositions, pronouns, verb forms in their everyday Roman-script spelling — `ka`, `hai`, `karein`, `mein`, and similar) and accepts text that's confidently Hindi-English code-mixed without ever asking `langdetect` at all for that case; `DetectorFactory.seed = 0` fixes the separate non-determinism problem for text that does still go through `langdetect`. Verified directly, not assumed: across a 30-message realistic Hinglish corpus (10 scam, 17 legit, 3 known-false-positive), the gate now accepts every single one, every time, across repeated trials — 0 rejections, fully deterministic (see `test_text_guard_accepts_realistic_hinglish_deterministically` in `tests/test_code_mixed_text.py`).

Fixing the gate surfaced a second, separate, **still-open** problem worth stating plainly rather than declaring victory too early: once Hinglish text reliably reaches the classifiers, the classifiers themselves are less accurate on it than on English. **3 of 10** realistic legit Hinglish messages are classified as scam confidently — not ambiguous ~0.5 calls the LLM escalation tier would catch — including, tellingly, a genuine OTP message whose own text is actively warning the reader not to share that OTP. The spend categorizer also confidently miscategorizes one legit Hinglish transaction (a salary credit, read as a P2P transfer). This is a model/training-data gap — both classifiers were trained on English-only labeled examples — not a gating problem, and needs a different fix (more Hinglish training data, or a lower escalation margin) than the one just applied. Tracked as deliberate, `strict` `xfail` tests in `tests/test_code_mixed_text.py` so these stay visible rather than getting silently re-masked now that the gate no longer hides them.
...
Full test suite: 209 tests passing, plus 4 deliberate `xfail`s (213 total) — see [Hardening: Hindi-English code-mixed text, CPU-only latency, and LLM offline re-verification](#hardening-hindi-english-code-mixed-text-cpu-only-latency-and-llm-offline-re-verification) above — `pytest` from the project root with the venv active. Nothing above touches Qualcomm AI Hub, an account, or a network call — see the appendix below if you want the Snapdragon-specific detail.
