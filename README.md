...
**Hindi-English code-mixed ("Hinglish") text.** Real Indian scam and transaction messages are routinely code-mixed, not clean English — so both classifiers were tested against realistic Hinglish, not just the English holdout/stress sets. The result is a genuine, measured gap, not a pass: `src/pipeline/text_guard.py` gates every message through `langdetect` before it ever reaches a model, restricted to English only. Across repeated trials on realistic Hinglish scam and transaction text, that gate rejected **46-72% of messages** as "not English" — most often misidentifying romanized Hindi as Indonesian, Estonian, or Turkish (short-text n-gram collisions), and **non-deterministically**: `langdetect` ships with no fixed random seed, so the identical string can pass on one call and get rejected on the next. When a message *does* get through, the scam classifier itself handles Hinglish well (0 misclassifications across every stable-passing scam example tested) — but one confirmed, repeatable exception stands out: a legit Hinglish refund message is classified as scam at **0.934 confidence**, not an ambiguous call the LLM escalation tier would catch, because the fast classifier is confidently wrong here, not unsure. The English phrasing of the same message is a known *ambiguous* case that correctly escalates — see `tests/test_code_mixed_text.py`, including a deliberately-failing (`xfail`) test that documents this false positive so it stays visible until it's actually fixed, rather than being silently forgotten. The spend categorizer's rejection rate on Hinglish was worse (~71% across measurement) than the scam classifier's, and its one consistently-passing ambiguous case (a salary-credit message) was misclassified as a P2P transfer every time. Bottom line: this is a real, current limitation of the English-only language gate on genuinely code-mixed input, not the underlying classifiers — documented here rather than glossed over.
...
├── tests/                      # 190 tests covering every module above
├── requirements.txt
└── README.md
```
...
Full test suite: 190 tests, all passing (plus 1 deliberate `xfail` — see [Hardening: Hindi-English code-mixed text, CPU-only latency, and LLM offline re-verification](#hardening-hindi-english-code-mixed-text-cpu-only-latency-and-llm-offline-re-verification) above) — `pytest` from the project root with the venv active. Nothing above touches Qualcomm AI Hub, an account, or a network call — see the appendix below if you want the Snapdragon-specific detail.
