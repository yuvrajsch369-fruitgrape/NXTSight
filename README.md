...
| **A real Snapdragon Windows PC (judges' hardware)** | **Read this before presenting live — the one thing this whole project hasn't been able to verify directly.** `torch==2.2.2` (every version checked, through 2.11.0) has **no Windows ARM64 wheel on PyPI at all** — confirmed with `pip download --platform win_arm64`, not assumed. If the judge's Python is the native ARM64 build, `pip install -r requirements.txt` will hard-fail on the torch line. **Fix:** install the regular 64-bit (x64/AMD64) Python instead of the ARM64 one — Windows 11 on ARM runs x64 Python fine via its built-in emulation, torch's `win_amd64` wheel installs normally, and Qualcomm does ship a `win_amd64` build of `onnxruntime-qnn` too (also confirmed downloadable), so QNN/NPU access should still work from an x64 process. That combination is genuinely **not verified end-to-end on physical hardware** — unlike the AI Hub compile+profile jobs elsewhere in this README, which really did run on physical Snapdragon silicon, just in Qualcomm's cloud device farm, not on a local machine. **If you can get even brief access to a real Snapdragon Windows PC before presenting, test the full install there first** — worst case if QNN genuinely doesn't activate, the app still runs correctly on the CPU fallback path (proven, not theoretical) with an honest on-screen "CPU" badge instead of "Snapdragon NPU," rather than breaking. |
