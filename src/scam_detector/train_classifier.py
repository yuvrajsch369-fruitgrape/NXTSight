...
dataset in data.py, then exports the classifier half to ONNX via skl2onnx
so it can run through the same QNN-aware runtime.py used for the OCR
models. Once this exact .onnx file is compiled for Snapdragon via AI Hub
(see scripts/aihub_compile_scam_classifier.py), it runs on the NPU
instead of CPU — no code change in classifier.py.
