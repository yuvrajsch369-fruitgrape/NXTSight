...
- **Execution path** — `select_execution_providers()` ([`src/pipeline/runtime.py`](../src/pipeline/runtime.py)) calls `onnxruntime.get_available_providers()` once and picks the best real accelerator it reports, in order: QNN (Snapdragon NPU), CUDA (NVIDIA GPU), OpenVINO (Intel NPU/GPU), DirectML (Windows GPU), CoreML (Apple Neural Engine/GPU), CPU otherwise. This dev machine is an Intel Mac with no Snapdragon NPU, but it does have Apple's CoreML provider — the response above is genuinely live, not a stand-in for what CPU would say. On a Snapdragon Windows PC, the same code takes the NPU branch with zero changes. QNN, CUDA, OpenVINO, and DirectML are each tested two levels deep without the physical hardware: `tests/test_runtime.py` mocks `get_available_providers()` to simulate each being present, then runs NXTSight's real `classifier.onnx` (not a synthetic graph) through the actual `create_inference_session()` code path and confirms it reaches the same verdict as every other path on the same input. What that can't prove without the real chip is that the acceleration itself is genuine — see the main README's [hardware-testing table](../README.md#what-runs-on-device-npu--gpu-vs-cpu-fallback) for the honest breakdown of what's real hardware versus simulated, path by path.
